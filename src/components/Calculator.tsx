...
export default function Calculator({ isOpen, onClose, onResult, initialValue = "" }: CalculatorProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/40 backdrop-blur-sm sm:items-center">
      <div
        className="w-full max-w-sm rounded-t-2xl bg-surface-container-low p-gutter shadow-2xl sm:rounded-2xl"
        onKeyDown={handleKeyDown}
      >
        <div className="mb-3 flex items-center justify-between">
          <span className="text-label-sm text-on-surface-variant/60">Kalkulator</span>
          <button
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-xl text-on-surface-variant/60 hover:bg-surface-container-highest hover:text-on-surface-variant transition-colors"
          >
        ...
        {expression && (
          <div className="mb-1 text-right text-label-sm text-on-surface-variant/40 h-4">
            {expression}
          </div>
        )}

        <div className="mb-4 overflow-hidden rounded-xl bg-surface-container-high/40 px-4 py-3 clay-inset">
          <input
            ref={inputRef}
            type="text"
            value={display}
            ...
            className="w-full bg-transparent text-right text-3xl font-bold text-on-surface outline-none"
          />
        </div>

        <div className="grid grid-cols-4 gap-stack-xs">
          <button onClick={clear} className={`${btnClass} bg-error-container/40 text-on-error-container hover:bg-error-container/60`}>
            C
          </button>
          <button onClick={backspace} className={`${btnClass} bg-surface-container-high/60 text-on-surface-variant/80 hover:bg-surface-container-high`}>
            <MaterialSymbol icon="backspace" size={18} />
          </button>
          <button onClick={() => performOperation("÷")} className={`${btnClass} ${operation === "÷" ? "bg-primary text-on-primary" : "bg-surface-container-high/60 text-on-surface-variant/80 hover:bg-surface-container-high"}`}>
            ÷
          </button>
          <button onClick={() => performOperation("×")} className={`${btnClass} ${operation === "×" ? "bg-primary text-on-primary" : "bg-surface-container-high/60 text-on-surface-variant/80 hover:bg-surface-container-high"}`}>
          ...
          {[7, 8, 9].map((n) => (
            <button key={n} onClick={() => inputDigit(n.toString())} className={`${btnClass} bg-surface-container-high/60 text-on-surface hover:bg-surface-container-high`}>
              {n}
            </button>
          ))}
          <button onClick={() => performOperation("-")} className={`${btnClass} ${operation === "-" ? "bg-primary text-on-primary" : "bg-surface-container-high/60 text-on-surface-variant/80 hover:bg-surface-container-high"}`}>
          ...
          <button onClick={() => inputDigit("0")} className={`${btnClass} bg-surface-container-high/60 text-on-surface hover:bg-surface-container-high col-span-2`}>
            0
          </button>
          <button onClick={inputDecimal} className={`${btnClass} bg-surface-container-high/60 text-on-surface hover:bg-surface-container-high`}>
            ,
          </button>
        </div>
        ...
        <div className="mt-stack-sm flex gap-stack-xs">
          <button
            onClick={onClose}
            className="flex-1 rounded-xl bg-surface-container-high/60 py-3 text-label-md font-medium text-on-surface-variant/60 hover:bg-surface-container-high transition-colors"
          >
            Batal
          </button>
          <button
